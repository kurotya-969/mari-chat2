...
class BackgroundProcessor:
    ...
    def _background_loop(self) -> None:
        """バックグラウンド処理のメインループ"""
        logger.info("バックグラウンド処理ループを開始します")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                error_msg = f"バックグラウンド処理ループでエラーが発生: {str(e)}"
                ...
                # エラー後は少し長めに待機
                self.stop_event.wait(min(self.check_interval * 2, 300))
        
        logger.info("バックグラウンド処理ループを終了しました")
