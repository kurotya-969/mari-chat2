...
class TutorialManager:
    ...
    def __init__(self):
        ...
        self.tutorial_steps = {
            ...
            6: {
                ...
                "description": "「カフェ」「神社」「水族館」などのキーワードを話すと、背景が動的に変わります。",
                "icon": "🎨",
                "target": "scene_change",
    ...
    def render_chat_tutorial_guide(self):
        ...
        # ステップ4で手紙タブにいる場合の特別メッセージ
        if current_step == 4:
            # 現在のタブを推測（完全ではないが、手紙タブでの表示を想定）
            st.info("👏 手紙タブに到達しました！下のフォームから手紙をリクエストしてみてください。")
    ...
    def _get_step_action_text(self, step_num: int) -> str:
        ...
        action_texts = {
            ...
            6: "「カフェに行きたい」「神社でお参りしたい」「水族館を見に行こう」などと話しかけて、背景の変化を楽しんでください。"
        }
    ...
    def check_step_completion(self, step_num: int, condition_met: bool):
        """ステップ完了条件をチェック"""
        if condition_met and not self.is_step_completed(step_num):
    ...
    def auto_check_completions(self):
        """自動的にステップ完了をチェック"""
        # ステップ1: メッセージ送信（初期メッセージを除外してカウント）
        ...
        # ステップ4: 手紙リクエスト
        # チュートリアル用手紙が生成されたかチェック
        if st.session_state.get('tutorial_letter_generated', False):
            self.check_step_completion(4, True)
